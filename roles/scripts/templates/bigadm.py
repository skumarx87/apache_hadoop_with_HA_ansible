...
class bigadm:
    ...
    def hdfs_service(self,action):
        if action == 'status':
            nn_pid_file = "{}/pids/hadoop-hadoop-namenode.pid".format(self.env_bigdata_root_value)
            ...
            for host in jn_hosts:
                print(host)
                self.check_process_of_pid_over_ssh(host,jn_pid_file,'java')
            for host in dn_hosts:
                print(host)
                self.check_process_of_pid_over_ssh(host,dn_pid_file,'java')
            '''
                ...
                if pid is not None:
                    ...
                    self.run_command_over_ssh(cmd)
            '''
            self.logger.info("name node pid : {}".format(nn_pid))
            if self.is_check_pid_running(nn_pid):
                self.logger.info("name node pid is running")
            else:
                self.logger.error("name node pid is not  running")
